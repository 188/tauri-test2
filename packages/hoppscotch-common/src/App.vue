<template>
  <div>
    ...
    <RouterView v-else />
    <Toaster rich-colors />
    <AppAbout :show="shouAbout" @hide-modal="shouAbout = false" />
  </div>
</template>
...
import { ipcListener, listenGlobalAbout } from "~/helpers/ipc"

const t = useI18n()
const shouAbout = ref(false)
...
const shouwwAbout = () => {
  shouAbout.value = true
}

ipcListener(router, toast)
listenGlobalAbout(shouwwAbout)
</script>
